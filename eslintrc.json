...
import { Copy, Megaphone, ClipboardList, Users, FolderOpen, LogOut, Trash2, ExternalLink, Sparkles, CheckCircle2, Hourglass, FileText, Youtube, Link2, ListChecks, PlusCircle } from 'lucide-react';
import {
  signOut,
  postAnnouncement,
  createAssignment,
...
export default function TeacherDashboard({ profile, classe, classes, announcements, assignments, members, resources, submissions }) {
  ...
  const [nouvelleClasse, setNouvelleClasse] = useState(null);
  ...
  async function handleCreateClasse(e) {
    ...
    if (result?.error) {
      ...
      setModal(null);
      e.target.reset();
      setNouvelleClasse(result.classe);
    }
  }
    ...
    <div>
      <nav className="barre-nav">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          {classes.length > 1 && (
            <select
              value={classe.id}
              onChange={(e) => router.push('/enseignant?classe=' + e.target.value)}
      ...
      <div className="contenu-principal">
        ...
        <div className="onglets">
          ...
          <div className="onglet" onClick={() => setModal('classe')} style={{ marginLeft: 'auto' }}>
            <PlusCircle size={15} /> Créer une nouvelle classe
          </div>
        </div>
        ...
        <div className="modale-fond" onClick={(e) => e.target === e.currentTarget && setModal(null)}>
          <div className="modale">
            ...
            <form onSubmit={handleCreateClasse}>
              <div className="champ"><label>Nom de la classe</label><input type="text" name="name" placeholder="ex. FLS avancé — hiver 2026" required /></div>
              <div className="champ">
                <label>Niveau</label>
              ...
              <div className="modale-actions">
                <button type="button" className="btn-sm btn-secondaire" onClick={() => setModal(null)}>Annuler</button>
                <button type="submit" className="btn-sm btn-primaire">Créer la classe</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {nouvelleClasse && (
        <div className="modale-fond" onClick={(e) => e.target === e.currentTarget && setNouvelleClasse(null)}>
          <div className="modale">
            <h3>✅ Classe créée !</h3>
            <p style={{ fontSize: '0.9rem', color: 'var(--gris)' }}>Partagez ce code avec vos apprenants pour « {nouvelleClasse.name} » :</p>
            <div className="code-classe">{nouvelleClasse.code}</div>
            <div className="modale-actions">
              <button className="btn-sm btn-secondaire" onClick={() => setNouvelleClasse(null)}>Fermer</button>
              <button
                className="btn-sm btn-primaire"
                onClick={() => { router.push('/enseignant?classe=' + nouvelleClasse.id); setNouvelleClasse(null); }}
              >
                Aller à cette classe
              </button>
            </div>
          </div>
        </div>
